...
@Slf4j
@Service
public class StillingskodeCacheService extends CacheService<FintResource<Stillingskode>> {
    ...
    public StillingskodeCacheService() {
        super(MODEL, KodeverkActions.GET_ALL_STILLINGSKODE);
    }

    @PostConstruct
    public void init() {
        Arrays.stream(props.getOrgs()).forEach(orgId -> {
            FintCache<FintResource<Stillingskode>> cache = new FintCache<>();
            put(orgId, cache);
        });
    }

    @Scheduled(initialDelayString = ConsumerProps.CACHE_INITIALDELAY_STILLINGSKODE, fixedRateString = ConsumerProps.CACHE_FIXEDRATE_STILLINGSKODE)
    ...
    public Optional<FintResource<Stillingskode>> getStillingskode(String orgId, String systemId) {
        return getOne(orgId, (fintResource) -> fintResource.getResource().getSystemId().getIdentifikatorverdi().equals(systemId));
    }

    @Override
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Stillingskode>>>() {
        });
    }
}
