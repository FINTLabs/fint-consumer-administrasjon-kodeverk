...
@Slf4j
@Api(tags = {"Stillingskode"})
@CrossOrigin
@RestController
@RequestMapping(name = "Stillingskode", value = RestEndpoints.STILLINGSKODE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class StillingskodeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Stillingskode cache is disabled.");
        }
    ...
    @GetMapping
    public StillingskodeResources getStillingskode(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getStillingskodeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Stillingskode cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<StillingskodeResource> resources;
    ...
    @PostMapping("/$query")
    public StillingskodeResources getStillingskodeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getStillingskode(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private StillingskodeResources getStillingskodeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_STILLINGSKODE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new StillingskodeResources();
    
        ArrayList<StillingskodeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<StillingskodeResource>>() {});
    ...
    public StillingskodeResource getStillingskodeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<StillingskodeResource> stillingskode = cacheService.getStillingskodeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return stillingskode.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            StillingskodeResource stillingskode = objectMapper.convertValue(response.getData().get(0), StillingskodeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(stillingskode);
        }    
    }
