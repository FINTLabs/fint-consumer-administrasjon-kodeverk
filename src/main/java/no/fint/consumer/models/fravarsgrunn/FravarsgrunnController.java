...
public class FravarsgrunnController {
    ...
    @GetMapping
    public FravarsgrunnResources getFravarsgrunn(
        ...
        List<FravarsgrunnResource> fravarsgrunn;
        ...
        return linker.toResources(fravarsgrunn);
    }


    @GetMapping("/systemid/{id:.+}")
    public FravarsgrunnResource getFravarsgrunnBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<FravarsgrunnResource> fravarsgrunn = cacheService.getFravarsgrunnBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fravarsgrunn.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FravarsgrunnResource fravarsgrunn = objectMapper.convertValue(response.getData().get(0), FravarsgrunnResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.toResource(fravarsgrunn);
        }    
    }
