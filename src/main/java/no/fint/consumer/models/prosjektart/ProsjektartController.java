...
@Slf4j
@Api(tags = {"Prosjektart"})
@CrossOrigin
@RestController
@RequestMapping(name = "Prosjektart", value = RestEndpoints.PROSJEKTART, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ProsjektartController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private ProsjektartCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private ProsjektartLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Prosjektart cache is disabled.");
        }
    ...
    @GetMapping
    public ProsjektartResources getProsjektart(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getProsjektartByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Prosjektart cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_PROSJEKTART, client);
        ...
        Stream<ProsjektartResource> resources;
    ...
    @PostMapping("/$query")
    public ProsjektartResources getProsjektartByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getProsjektart(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ProsjektartResources getProsjektartByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_PROSJEKTART, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ProsjektartResources();
    
        ArrayList<ProsjektartResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ProsjektartResource>>() {});
    ...
    public ProsjektartResource getProsjektartBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_PROSJEKTART, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<ProsjektartResource> prosjektart = cacheService.getProsjektartBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return prosjektart.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ProsjektartResource prosjektart = objectMapper.convertValue(response.getData().get(0), ProsjektartResource.class);
