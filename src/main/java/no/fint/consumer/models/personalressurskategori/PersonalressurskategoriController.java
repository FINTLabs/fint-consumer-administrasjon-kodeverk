...
@Slf4j
@Api(tags = {"Personalressurskategori"})
@CrossOrigin
@RestController
...
public class PersonalressurskategoriController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Personalressurskategori cache is disabled.");
        }
    ...
    @GetMapping
    public PersonalressurskategoriResources getPersonalressurskategori(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getPersonalressurskategoriByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Personalressurskategori cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<PersonalressurskategoriResource> resources;
    ...
    @PostMapping("/$query")
    public PersonalressurskategoriResources getPersonalressurskategoriByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getPersonalressurskategori(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private PersonalressurskategoriResources getPersonalressurskategoriByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_PERSONALRESSURSKATEGORI, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new PersonalressurskategoriResources();
    
        ArrayList<PersonalressurskategoriResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<PersonalressurskategoriResource>>() {});
    ...
    public PersonalressurskategoriResource getPersonalressurskategoriBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<PersonalressurskategoriResource> personalressurskategori = cacheService.getPersonalressurskategoriBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return personalressurskategori.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            PersonalressurskategoriResource personalressurskategori = objectMapper.convertValue(response.getData().get(0), PersonalressurskategoriResource.class);
