...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.personalressurskategori", havingValue = "false", matchIfMissing = true)
    ...
    public void onAction(Event event) {
        ...
        if (KodeverkActions.valueOf(event.getAction()) == KodeverkActions.UPDATE_PERSONALRESSURSKATEGORI) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<PersonalressurskategoriResource>> cacheObjects = data
                    .stream()
