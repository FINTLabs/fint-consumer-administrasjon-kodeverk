...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.lonnsart", havingValue = "false", matchIfMissing = true)
    ...
    public void onAction(Event event) {
        ...
        if (KodeverkActions.valueOf(event.getAction()) == KodeverkActions.UPDATE_LONNSART) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<LonnsartResource>> cacheObjects = data
                    .stream()
