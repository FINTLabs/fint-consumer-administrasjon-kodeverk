...
@CrossOrigin
@RestController
@RequestMapping(name = "Lonnsart", value = RestEndpoints.LONNSART, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class LonnsartController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Lonnsart cache is disabled.");
        }
    ...
    @GetMapping
    public LonnsartResources getLonnsart(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getLonnsartByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Lonnsart cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<LonnsartResource> resources;
    ...
    @PostMapping("/$query")
    public LonnsartResources getLonnsartByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getLonnsart(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private LonnsartResources getLonnsartByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_LONNSART, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new LonnsartResources();
    
        ArrayList<LonnsartResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<LonnsartResource>>() {});
    ...
    public LonnsartResource getLonnsartBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<LonnsartResource> lonnsart = cacheService.getLonnsartBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return lonnsart.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            LonnsartResource lonnsart = objectMapper.convertValue(response.getData().get(0), LonnsartResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(lonnsart);
        }    
    }
