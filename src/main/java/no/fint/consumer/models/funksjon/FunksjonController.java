...
public class FunksjonController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Funksjon cache is disabled.");
        }
    ...
    public FunksjonResources getFunksjon(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFunksjonByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Funksjon cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<FunksjonResource> resources;
    ...
    @PostMapping("/$query")
    public FunksjonResources getFunksjonByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFunksjon(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FunksjonResources getFunksjonByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FUNKSJON, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FunksjonResources();
    
        ArrayList<FunksjonResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FunksjonResource>>() {});
    ...
    public FunksjonResource getFunksjonBySystemId(
        ...
        if (cacheService != null) {
            ...
            FunksjonResource funksjon = objectMapper.convertValue(response.getData().get(0), FunksjonResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(funksjon);
        }    
    }
