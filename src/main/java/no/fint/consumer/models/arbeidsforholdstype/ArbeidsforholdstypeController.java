...
@Slf4j
@Api(tags = {"Arbeidsforholdstype"})
@CrossOrigin
@RestController
@RequestMapping(name = "Arbeidsforholdstype", value = RestEndpoints.ARBEIDSFORHOLDSTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ArbeidsforholdstypeController {
    ...
    @Autowired
    private ArbeidsforholdstypeLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Arbeidsforholdstype cache is disabled.");
        }
    ...
    @GetMapping
    public ArbeidsforholdstypeResources getArbeidsforholdstype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getArbeidsforholdstypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Arbeidsforholdstype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<ArbeidsforholdstypeResource> resources;
    ...
    @PostMapping("/$query")
    public ArbeidsforholdstypeResources getArbeidsforholdstypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getArbeidsforholdstype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ArbeidsforholdstypeResources getArbeidsforholdstypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_ARBEIDSFORHOLDSTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ArbeidsforholdstypeResources();
    
        ArrayList<ArbeidsforholdstypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ArbeidsforholdstypeResource>>() {});
    ...
    public ArbeidsforholdstypeResource getArbeidsforholdstypeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<ArbeidsforholdstypeResource> arbeidsforholdstype = cacheService.getArbeidsforholdstypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return arbeidsforholdstype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ArbeidsforholdstypeResource arbeidsforholdstype = objectMapper.convertValue(response.getData().get(0), ArbeidsforholdstypeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(arbeidsforholdstype);
        }    
    }
