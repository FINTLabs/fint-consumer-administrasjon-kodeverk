...
@CrossOrigin
@RestController
@RequestMapping(name = "Ansvar", value = RestEndpoints.ANSVAR, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class AnsvarController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Ansvar cache is disabled.");
        }
    ...
    @GetMapping
    public AnsvarResources getAnsvar(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getAnsvarByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Ansvar cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<AnsvarResource> resources;
    ...
    @PostMapping("/$query")
    public AnsvarResources getAnsvarByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getAnsvar(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private AnsvarResources getAnsvarByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_ANSVAR, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new AnsvarResources();
    
        ArrayList<AnsvarResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<AnsvarResource>>() {});
    ...
    public AnsvarResource getAnsvarBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<AnsvarResource> ansvar = cacheService.getAnsvarBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return ansvar.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            AnsvarResource ansvar = objectMapper.convertValue(response.getData().get(0), AnsvarResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(ansvar);
        }    
    }
