...
public class UketimetallController {
    ...
    @GetMapping
    public ResponseEntity getUketimetall(
    ...
    public ResponseEntity getUketimetallBySystemId(@PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client) {
        if (props.isOverrideOrgId() || orgId == null) {
            orgId = props.getDefaultOrgId();
        ...
        Optional<FintResource<Uketimetall>> uketimetall = cacheService.getUketimetallBySystemId(orgId, id);
