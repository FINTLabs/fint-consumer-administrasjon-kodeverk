...
public class ProsjektController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Prosjekt cache is disabled.");
        }
    ...
    public ProsjektResources getProsjekt(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getProsjektByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Prosjekt cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<ProsjektResource> resources;
    ...
    @PostMapping("/$query")
    public ProsjektResources getProsjektByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getProsjekt(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ProsjektResources getProsjektByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_PROSJEKT, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ProsjektResources();
    
        ArrayList<ProsjektResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ProsjektResource>>() {});
    ...
    public ProsjektResource getProsjektBySystemId(
        ...
        if (cacheService != null) {
            ...
            ProsjektResource prosjekt = objectMapper.convertValue(response.getData().get(0), ProsjektResource.class);
